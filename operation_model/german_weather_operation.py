...
class GermanWeatherOperation(Operation):
    ...
    def perform_operation(self, prefix_path='/pub/CDC/observations_germany/climate/'):
        try:
            ...
            paths_to_parse = ['/pub/CDC/observations_germany/climate/subdaily/standard_format/KL_Standardformate_Beschreibung_Stationen.txt']
            if paths_to_parse:
                for path in paths_to_parse:
                    try:
                        ...
                        if Helper.is_txt_file(path):
                            if not Helper.is_station(path):
                                continue
                            file_path = path
                        else:
